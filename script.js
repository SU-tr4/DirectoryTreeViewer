document.addEventListener('DOMContentLoaded', () => {
  ...
  copyBtn.addEventListener('click', () => {
    ...
    if (!textToCopy || textToCopy === 'ここにツリー構造が出力されます...') return;
  ...
  function processFiles(files) {
    rawFilesCache = Array.from(files);
    renderOutput();
  }
  ...
  function renderOutput() {
    ...
    for (const file of rawFilesCache) {
      ...
      if (isIgnoreGit && path.includes('.git/')) continue;
      if (isIgnoreModules && path.includes('node_modules/')) continue;
